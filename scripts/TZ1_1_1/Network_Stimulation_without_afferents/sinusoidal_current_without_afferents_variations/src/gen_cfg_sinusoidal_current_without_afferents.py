...
PERIOD_RANGE = np.linspace(1, 1000, 8)
PHASE_RANGE = np.linspace(0, 1, 8)
        ...
        for period0, period1 in product(PERIOD_RANGE, repeat=2):
            for phase0, phase1 in product(PHASE_RANGE, repeat=2):
                record = {
                    ...
                    # Группа 0 (каналы 0 и 2) - синусоидальный ток
                    "sine_period_ch0": period0,
                    "sine_period_ch2": period0,
                    "amplitude_ch0": 5,
                    "amplitude_ch2": 5,
                    "phase_ch0": phase0,
                    "phase_ch2": phase0,
                    
                    # Группа 1 (каналы 1 и 3) - синусоидальный ток
                    "sine_period_ch1": period1,
                    "sine_period_ch3": period1,
                    "amplitude_ch1": 4,
                    "amplitude_ch3": 4,
                    "phase_ch1": phase1,
                    "phase_ch3": phase1,
...
sine_columns = [
    'sine_period_ch0', 'sine_period_ch2', 'amplitude_ch0', 'amplitude_ch2', 'phase_ch0', 'phase_ch2',
    'sine_period_ch1', 'sine_period_ch3', 'amplitude_ch1', 'amplitude_ch3', 'phase_ch1', 'phase_ch3',
...
other_columns = sorted([col for col in df.columns if col not in base_columns + sine_columns + weight_columns])

df = df[base_columns + sine_columns + weight_columns + other_columns]
...
sine_combinations = len(list(product(PERIOD_RANGE, repeat=2))) * len(list(product(PHASE_RANGE, repeat=2)))
...
print(f"  - Вариаций синусоидальных параметров: {sine_combinations}")
